...
export default function SearchSidebar({ locations, cuisine, searchParams }: { locations: Location[], cuisine: Cuisine[]; searchParams: { city?: string, cuisine?: string, price?: PRICE } }) {
    const prices = [
        {
            price: PRICE.CHEAP,
            label: "$$",
                ...
                <h1 className="mb-2">Cuisine</h1>
                {
                    cuisine.map((cus) => {
                        return (
                            ...
                            }} className="font-light text-reg capitalize" key={cus.id}>{cus.name}</Link>
                        )
                    })
                }
            </div>
